"""Cross-platform automation helpers for Raptor (Windows-compatible)."""
...
def _run_osascript(script: str, env: dict[str, str] | None = None, timeout: int = 20) -> subprocess.CompletedProcess:
    if os.name == 'nt':
        return subprocess.CompletedProcess(args=["osascript"], returncode=1, stderr="Automation is not supported on Windows.", stdout="")
...
def open_whatsapp() -> str:
    """Open the WhatsApp desktop app (Windows-compatible)."""
    if os.name == 'nt':
        import shutil, webbrowser as _wb
        # Try WhatsApp UWP (Microsoft Store)
        wa_store = shutil.which("WhatsApp.exe")
        if wa_store:
            try:
                subprocess.Popen([wa_store], creationflags=subprocess.DETACHED_PROCESS)
                return "WhatsApp is now open."
            except Exception:
                pass
        # Fallback: open WhatsApp Web
        _wb.open("https://web.whatsapp.com")
