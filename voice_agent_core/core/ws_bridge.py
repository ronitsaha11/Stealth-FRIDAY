...
class RaptorWSBridge:
    ...
    async def _handle_client_message(self, websocket, message):
        """Handle dashboard commands sent over the bridge."""
        try:
        ...
        if not self.command_handler:
            await websocket.send(json.dumps({
                "type": "command_ack",
                "ok": False,
                "message": "No command handler is registered.",
            }))
            return

        try:
            result = self.command_handler(payload) or {}
            await websocket.send(json.dumps({
                "type": "command_ack",
            ...
            logger.exception("[WS] Command handler failed")
            await websocket.send(json.dumps({
                "type": "command_ack",
                "ok": False,
                "message": str(exc),
    ...
    def set_command_handler(self, handler):
        """Register a callback for dashboard-originated commands."""
