...
class BrowserBridge:
    ...
    def start(self):
        ...
        def free_port(port: int):
            """Kill any process listening on the given port (Windows-compatible via psutil)."""
            try:
                import psutil
                    ...
                    if conn.laddr.port == port and conn.status == "LISTEN":
                        ...
                        if pid:
                            try:
                                ...
                                logger.info(f"[DEBUG] Port {port} in use by PID {pid} ({proc.name()}). Killing.")
                                proc.kill()
                                time.sleep(0.5)
                            except (psutil.NoSuchProcess, psutil.AccessDenied) as kill_err:
                                logger.warning(f"[DEBUG] Could not kill PID {pid}: {kill_err}")
            except Exception as e:
