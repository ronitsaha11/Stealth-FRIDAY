...
def _action_show_top_cpu(event: dict) -> dict:
    """List top 5 CPU-consuming processes (Windows-compatible via psutil)."""
    try:
        import psutil
        procs = []
        for p in psutil.process_iter(["pid", "name", "cpu_percent"]):
            try:
                ...
                procs.append((cpu, p.info["name"] or "unknown"))
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        procs.sort(reverse=True)
        top = procs[:5]
        lines = [f"{name} at {cpu:.1f}%" for cpu, name in top]
        summary = ", ".join(lines[:3]) if lines else "No processes found"
        return {
            "status": "success",
...
def _action_show_top_ram(event: dict) -> dict:
    """List top 5 memory-consuming processes (Windows-compatible via psutil)."""
    try:
        import psutil
        ...
        for p in psutil.process_iter(["pid", "name", "memory_info"]):
            try:
                ...
                mem_pct = (mem_bytes / total_ram) * 100 if total_ram else 0
                procs.append((mem_pct, p.info["name"] or "unknown"))
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        procs.sort(reverse=True)
        top = procs[:5]
        lines = [f"{name} at {pct:.1f}%" for pct, name in top]
        summary = ", ".join(lines[:3]) if lines else "No processes found"
        return {
            "status": "success",
...
def _action_disk_report(event: dict) -> dict:
    """Report large files in Windows Temp and Downloads folders (read-only, no deletion)."""
    import os
    try:
        search_dirs = [
            os.environ.get("TEMP", ""),
        ...
        if count > 0:
            msg = f"Found {count} large file{'s' if count != 1 else ''} over 100MB in temp/downloads directories."
        else:
...
class RaptorMonitor:
    ...
    def _speak_alert_only(self, event: dict) -> None:
        ...
        Always restores IDLE state when done so wake word keeps working.
        """
        alert_text = event["message"]
        if event.get("suggestion"):
            alert_text += f" {event['suggestion']}"
        ...
        # CRITICAL: always return to IDLE so wake word keeps working
    ...
    def _monitor_loop(self) -> None:
        ...
        # Wait before first check to let agent fully initialize and speak greeting
        # Also pre-seed last-check times so external checks don't ALL fire at t=0
        self._stop_event.wait(timeout=45)
        if self._stop_event.is_set():
            return

        now = time.time()
        # Stagger external checks so they don't pile up on first iteration
        self._last_weather_check = now          # weather: fire after WEATHER_CHECK_INTERVAL
        self._last_sports_check  = now - 60     # sports:  fire after ~60s
        self._last_news_check    = now - 90     # news:    fire after ~90s
