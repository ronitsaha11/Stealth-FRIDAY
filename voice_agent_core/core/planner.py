...
def _plan_task_internal(user_input: str) -> list:
    ...
    # ── Strip commas Whisper inserts in natural speech ──────────────────────
    # e.g. "Open, chrome." → "open chrome"  |  "Open, task manager." → "open task manager"
    # We only do this BEFORE blocklist/keyword checks so multi-step "X and Y" still works
    ...
    # Normalize: strip remaining punctuation for reliable keyword matching
    ...
    # ── Direct app-open by name (Windows-aware) ──
    # Checked BEFORE the URL handler to avoid routing known apps through browser
    KNOWN_APPS = [
        # System tools
        ...
        "discord", "slack", "zoom",
        "whatsapp", "telegram",
        "calendar", "clock", "music",
    ]
    open_app_match = re.search(r"(?:open|launch|start)\s+(.+)", s_clean)
    if open_app_match:
        app_candidate = open_app_match.group(1).strip().rstrip(".,?!")
        if app_candidate and any(known in app_candidate or app_candidate in known for known in KNOWN_APPS):
            return [{"tool": "open_app", "args": {"app_name": app_candidate}}]
    ...
    plan = []
    # Split only on " and " / " then " — NOT on commas (commas are stripped above)
    ...
    for step in steps:
        if "create folder" in step or "create a folder named" in step:
            ...
            # Guard: skip if nothing came after "open" (prevents empty app_name)
            if app:
                plan.append({"tool": "open_app", "args": {"app_name": app}})
