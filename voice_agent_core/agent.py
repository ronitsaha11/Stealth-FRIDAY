...
# ── Force UTF-8 output so emoji / unicode arrows don't crash on Windows ──
...
class LocalVoiceAgent:
    ...
    def _handle_dashboard_command(self, payload: dict):
        command = payload.get("command")

        if command == "wake":
            ws_bridge.update_state({
                "last_response": "Dashboard wake requested.",
                "active_module": "core",
            })
            if self.state == "IDLE":
                self.wake_event.set()
                return {"ok": True, "message": "Wake signal sent."}

            return {"ok": True, "message": f"Raptor is already {self.state.lower()}."}

        return {"ok": False, "message": f"Unknown dashboard command: {command}"}
        ...
        if not is_port_in_use(3000):
            ...
            try:
                ...
                if sys.platform == "win32":
                    # Windows: look for .bat launcher
                    frontend_script = os.path.join(project_root, "launch_frontend.bat")
                    if os.path.exists(frontend_script):
                        subprocess.Popen(
                            ["cmd", "/c", "start", "", frontend_script],
                    ...
                    # macOS / Linux: look for .sh launcher
                    frontend_script = os.path.join(project_root, "frontend", "launch_frontend.sh")
                    if os.path.exists(frontend_script):
                        subprocess.Popen(
    ...
    def run(self):
        ...
        lock_file_path = os.path.join(temp_dir, "raptor_agent.lock")
        ...
        # ── STAGGERED STARTUP — prevents simultaneous heavy RAM allocation ──
        # On Windows, loading OWW + Whisper + all bridges at once can spike
        # RAM by 1–2 GB in under a second, causing a system hang / BSOD.

        # 2. Start WebSocket Bridge (lightweight)
        print("[STARTUP] Starting WebSocket bridge...")
        ws_bridge.set_command_handler(self._handle_dashboard_command)
        ws_bridge.start()
        time.sleep(0.5)

        # 3. Start Browser Intelligence Bridge (lightweight)
        print("[STARTUP] Starting Browser bridge...")
        browser_bridge.start()
        time.sleep(0.5)

        # 4. Start proactive background monitoring daemon (lightweight, 45s warmup)
        print("[STARTUP] Starting monitor daemon...")
        self.monitor.start()
        time.sleep(0.5)

        # 5. Pre-warm the Whisper STT model in a background thread BEFORE the
        #    wake listener starts, so both don't race to allocate memory at once.
        print("[STARTUP] Pre-warming STT model in background...")
        def _prewarm_stt():
            from core.local_audio import get_stt_model
            get_stt_model()   # loads once, cached globally
        prewarm_thread = threading.Thread(target=_prewarm_stt, daemon=True, name="STTPrewarm")
        prewarm_thread.start()

        # Speak greeting WHILE Whisper warms up in background (non-blocking)
        self.set_state("SPEAKING")
        speak("Raptor local systems online. Loading voice models, please wait.", blocking=True)

        # Wait for STT model to finish loading before starting wake listener
        prewarm_thread.join(timeout=60)
        print("[STARTUP] STT model ready.")

        # 6. Start wake word listener AFTER STT model is loaded (they share the model)
        print("[STARTUP] Starting wake word listener...")
        ...
        wake_thread.start()
        time.sleep(1.0)   # let OWW model finish loading before the loop starts

        speak("Waiting for wake word.", blocking=True)
        self.set_state("IDLE")
...
if __name__ == "__main__":
    ...
    # (even though we replaced most uses with threads, some deps may still need it)
